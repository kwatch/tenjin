...
spec_of "Tenjin::MemoryBaseStore", sub {
    ...
    spec_of "#get()", sub {
        ...
        it "returns nothing if cache data doen't exist", sub {
            ok ! $store->get('hogehogehoge');
        };

        it "returns cached value if it exists", sub {
            is $store->get($key), $value;
        };
...
spec_of "Tenjin::FileBaseStore", sub {
    ...
    spec_of "#set()", sub {
        ...
        it "sets cache file timestamp to 1 week ahead if lifetime is not specified", sub {
            is((stat $cache_fpath)[9], time() + $Tenjin::FileBaseStore::LIFE_TIME);
        };

        it "sets cache file timestamp to lifetime seconds ahead if it is specified", sub {
            $store->set($key, $value, 10);
            is((stat $cache_fpath)[9], time() + 10);
        };
    ...
    spec_of "#render()", sub {
        ...
        it "calls context block when cache is expired", sub {
            pre_task {
                ...
                ( (stat $cache_path)[9] + 5*60 - 1 < time() )  or die;
            };
            my $block_called = 0;
            my $context = {
                user => undef,
